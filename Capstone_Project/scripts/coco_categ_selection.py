...
class coco_category_filter:
    ...
    def __init__(self, json_path, imgs_dir, categ='cell phone'):
    ...
    def get_imgs_from_json(self):
        ...
        print(
            f"{len(images)} images in '{self.json_path}' with '{self.categ}' instances")
    ...
    def filter_json_by_category(self, new_json_path):
        ...
        # get img_ids of imgs with the category
        imgs_ids = [x['id'] for x in self.images]
        new_imgs = [x for x in self.coco.dataset['images']
                    if x['id'] in imgs_ids]
        catIds = self.catIds
        # Filter annotations
        new_annots = [x for x in self.coco.dataset['annotations']
                      if x['category_id'] in catIds]
        # Reorganize the ids
        new_imgs, annotations = self.modify_ids(new_imgs, new_annots)
        # Filter categories
        new_categories = [
            x for x in self.coco.dataset['categories'] if x['id'] in catIds]
    ...
    def modify_ids(self, images, annotations):
        ...
        for n, im in enumerate(images):
            # dicto with old im_ids and new im_ids
        ...
        for n, ann in enumerate(annotations):
            ...
            # replace im_ids in the annotations as well
            annotations[n]['image_id'] = old_new_imgs_ids[old_image_id]
        return images, annotations


def main(subset, year, root_dir, category='cell phone'):
    json_file = join(os.path.split(root_dir)[
                     0], 'instances_'+subset+year+'.json')   # local path
    imgs_dir = join(root_dir, category + '_' + subset)
    new_json_file = join(root_dir, 'annotations', subset+".json")
    coco_filter = coco_category_filter(
        json_file, imgs_dir, categ=category)  # instanciate class
